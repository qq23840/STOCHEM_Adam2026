...
C   NBLOCK:  No. of cells treated per block in structured calculation.
      ...
      IMPLICIT NONE
      INCLUDE '../model_files/INOPERVM10_sensitivity_PhotHi.SOA2'
...
C----------------------------------------------------------------------
      ...
      REAL XGRID,RANDNUM
      REAL Q,R,S,DETA
      INTEGER I,J,K,K0,L,N,MLEV(NLEV),SEED
      DOUBLE PRECISION NN(MNLAT,NLEV)
      RAD=PI/180.0
...
C For PhotHi run, the low-pressure limit of the product yield is used. 
C
...
C
C Calculate CF3CHO product yields (only once on first level)
C
          IF (J.EQ.1) THEN
            CALL CALC_CF3CHO_QY(NLEV, NLAM, NN, LAMBDA,
     &                          PCF3CHOa, PCF3CHOb)
...
C
C PhotHi variant: uses zero pressure limit in QY formula
      ...
      DO I=26,29
        DO J=1,NLEV
          PCF3CHOa(I,J)=(1-(1.951E-21*NN(9) - 3.0182E-41*(NN(9)**2))
     &                  *(LAMBDA(I)-266))
     &                  *0.5*(1-(0.9999999/(1+EXP((272-LAMBDA(I))
     &                  /(7-4.03E-23*NN(J))))))
          PCF3CHOb(I,J)=(1-(1.951E-21*NN(9) - 3.0182E-41*(NN(9)**2))
     &                  *(LAMBDA(I)-266))
     ...
     &                  /(7-4.03E-23*NN(J)))))))
      ...
      DO I=30,NLAM
        DO J=1,NLEV
          PCF3CHOa(I,J)=(1+NN(9)*5.8*1.0E-18*EXP(-0.0656*(360-
     &                  LAMBDA(I))))**(-1)
     &                  *0.5*(1-(0.9999999/(1+EXP((272-LAMBDA(I))
     &                  /(7-4.03E-23*NN(J))))))
          PCF3CHOb(I,J)=(1+NN(9)*5.8*1.0E-18*EXP(-0.0656*(360-
     &                  LAMBDA(I))))**(-1)
     &                  *(1-0.5*(1-(0.9999999/(1+EXP((272-LAMBDA(I))
     &                  /(7-4.03E-23*NN(J)))))))
        END DO
