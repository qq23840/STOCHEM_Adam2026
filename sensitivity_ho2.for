...
C   NBLOCK:  No. of cells treated per block in structured calculation.
      ...
      IMPLICIT NONE
      INCLUDE '../model_files/INOPERVM10_sensitivity_ho2.SOA2'
...
C           CF3CHO            Y(JB,250)         chemical production from HFCs and HFOs
     ...
     &+(RC(JB,592)*Y(JB,256))
C           LOSS PROCESSES ARE DEPOSITION, OH REACTION AND PHOTOLYSIS AND HO2 REACTION
...
C           CF3CHO2HO2      Y(JB,256)
          P = RC(JB,591)*Y(JB,250)*Y(JB,9)
          L = RC(JB,592)
...
C        CF3CHO + HO2 = CF3CHO2HO
      FLUX(JB,593)=FLUX(JB,593)+RC(JB,591)*Y(JB,9)*Y(JB,250)*DTS/M(JB)
C        CF3CHO2HO = CF3CHO + HO2
      FLUX(JB,594)=FLUX(JB,594)+RC(JB,592)*Y(JB,256)*DTS/M(JB)
...
C     Reaction (591) : HO2 + CF3CHO = CF3CHO2HO (reversible reaction with HO2, Long et al., 2022)
        RC(JB,591) = 2.48E-13
C     Reaction (592) : CF3CHO2HO -> HO2 + CF3CHO (reversible reaction, Long et al., 2022, high pressure limit at 298K)
        RC(JB,592) = 1.73E3
...
C NH3 (man-related)
     ...
     &       /(14.0*31526000.0)
C HFO1234yf (anthro)
          EMISS(230,I,J)=(CLASS(1,230)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(114.0*31536000.0)
C cHFO1225ye (anthro)
          EMISS(231,I,J)=(CLASS(1,231)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(120.0*31536000.0)
C tHFO1225ye (anthro)
          EMISS(232,I,J)=(CLASS(1,232)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(120.0*31536000.0)
C cHFO1233zd (anthro)
          EMISS(233,I,J)=(CLASS(1,233)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(130.5*31536000.0)
C tHFO1233zd (anthro) MODIFIED: Back to HFCDAT setup for now
          EMISS(234,I,J)=(CLASS(1,234)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(130.5*31536000.0)
C cHFO1234ze (anthro)
          EMISS(235,I,J)=(CLASS(1,235)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(114.0*31536000.0)
C tHFO1234ze (anthro) MODIFIED: Back to HFCDAT setup for now
          EMISS(236,I,J)=(CLASS(1,236)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(114.0*31536000.0)
C cHFO1336mz (anthro) MODIFIED: Back to HFCDAT setup for now
          EMISS(237,I,J)=(CLASS(1,237)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(164.0*31536000.0)
C tHFO1336mz (anthro)
          EMISS(238,I,J)=(CLASS(1,238)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(164.0*31536000.0)
C cHFO1318my (anthro)
          EMISS(239,I,J)=(CLASS(1,239)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(200.0*31536000.0)
C tHFO1318my (anthro)
          EMISS(240,I,J)=(CLASS(1,240)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(200.0*31536000.0)
C HFO1243zf (anthro)
          EMISS(241,I,J)=(CLASS(1,241)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(96.0*31536000.0)
C HFO1438ez (anthro)
          EMISS(242,I,J)=(CLASS(1,242)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(214.0*31536000.0)
C HFO1233xf (anthro)
          EMISS(243,I,J)=(CLASS(1,243)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(130.5*31536000.0)
C HFO1216 (anthro)
          EMISS(244,I,J)=(CLASS(1,244)/0.0769)*HFCDAT(I1,J1)*1E3*NA
     &       /(150.0*31536000.0)
C HFC143a (anthro)
...
C
      ...
      OPEN(40,FILE=DATDIR//'chem10_soa_oh.dat',STATUS='OLD')
C      OPEN(40,FILE=datdir//'allchem.dat',STATUS='OLD')
   ...
   10 CONTINUE
      CLOSE(40)
C
      OPEN(41,FILE=DATDIR//'flux10_OH_HO2.dat',STATUS='OLD')
      NFLUX=0
      N3DFLUX=0
      IERR=0
