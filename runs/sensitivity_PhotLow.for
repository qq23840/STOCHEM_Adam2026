...
C   NBLOCK:  No. of cells treated per block in structured calculation.
      ...
      IMPLICIT NONE
      INCLUDE '../model_files/INOPERVM10_sensitivity_PhotLow.SOA2'
...
C For PhotLow run, the high-pressure limit of the product yield is used. 
C
...
C
C PhotLow variant: uses high-pressure-limit in QY)
      ...
      DO I=26,29
        DO J=1,NLEV
          PCF3CHOa(I,J)=(1-(1.951E-21*NN(1) - 3.0182E-41*(NN(1)**2))
     &                  *(LAMBDA(I)-266))
     &                  *0.5*(1-(0.9999999/(1+EXP((272-LAMBDA(I))
     &                  /(7-4.03E-23*NN(J))))))
          PCF3CHOb(I,J)=(1-(1.951E-21*NN(1) - 3.0182E-41*(NN(1)**2))
     &                  *(LAMBDA(I)-266))
     ...
     &                  /(7-4.03E-23*NN(J)))))))
      ...
      DO I=30,NLAM
        DO J=1,NLEV
          PCF3CHOa(I,J)=(1+NN(1)*5.8*1.0E-18*EXP(-0.0656*(360-
     &                  LAMBDA(I))))**(-1)
     &                  *0.5*(1-(0.9999999/(1+EXP((272-LAMBDA(I))
     &                  /(7-4.03E-23*NN(J))))))
          PCF3CHOb(I,J)=(1+NN(1)*5.8*1.0E-18*EXP(-0.0656*(360-
     &                  LAMBDA(I))))**(-1)
     &                  *(1-0.5*(1-(0.9999999/(1+EXP((272-LAMBDA(I))
     &                  /(7-4.03E-23*NN(J)))))))
        END DO
